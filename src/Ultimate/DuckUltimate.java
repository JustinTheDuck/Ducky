...
public class DuckUltimate {
    ...
    public void update() {
        updateHitbox();
        if(Ultimate.playerInfo[0] && gp.player.Character.equals("Duck")) {
            basicCounter++;
            if(!basicHit && Ultimate.player2Hitbox[1] >= basicHitbox[0] && Ultimate.player2Hitbox[0] <= basicHitbox[1] && Ultimate.player2Hitbox[3] >= basicHitbox[2] && Ultimate.player2Hitbox[2] <= basicHitbox[3]) {gp.player2.health -= DAMAGE; basicHit = true; gp.player.basicCOUNTER = 0;}
            //When to kill Punch
            if (life <= basicCounter) {gp.player.basicCOUNTER = 0; Ultimate.playerInfo[0] = false; basicCounter = 0; basicHit = false;}
        }
        if(Ultimate.playerInfo[1] && gp.player.Character.equals("Duck")) {
        ...
        //Player 2
        if(Ultimate.player2Info[0] && gp.player2.Character.equals("Duck")) {
            basicCounter2++;
            if(!basicHit2 && Ultimate.playerHitbox[1] >= basicHitbox2[0] && Ultimate.playerHitbox[0] <= basicHitbox2[1] && Ultimate.playerHitbox[3] >= basicHitbox2[2] && Ultimate.playerHitbox[2] <= basicHitbox2[3]) {gp.player.health -= DAMAGE; basicHit2 = true; gp.player2.basicCOUNTER = 0;}
            //When to kill Punch
            if (life <= basicCounter2) {gp.player2.basicCOUNTER = 0; Ultimate.player2Info[0] = false; basicCounter2 = 0; basicHit2 = false;}
        }
        if(Ultimate.player2Info[1] && gp.player2.Character.equals("Duck")) {
            //Update the max possible velocity of the player during ultimate
